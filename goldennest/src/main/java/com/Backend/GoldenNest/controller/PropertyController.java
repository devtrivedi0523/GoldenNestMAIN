...
public class PropertyController {
    ...
    public Page<PropertyCardDto> list(
        ...
        // free-text search: title / description / city / state
        if (q != null && !q.isBlank()) {
    ...
    public PropertyDetailDto get(@PathVariable Long id) {
        ...
        // ⭐ NEW: extra media lists (from CSV)
    ...
    public ResponseEntity<Map<String, Object>> create(@Valid @RequestBody PropertyCreateDto dto) {
        ...
        // ⭐ NEW: extra media from advanced form (saved as CSV)
        ...
        // ensure 'type' is also populated
        String typeValue = dto.getType();
        if (typeValue == null || typeValue.isBlank()) {
            typeValue = dto.getPropertyType();
        }
        p.setType(typeValue);
    ...
    public ResponseEntity<Map<String, Object>> updateAdvanced(
        ...
        // Extra media (store as CSV in entity)
        p.setFloorPlans(joinCsv(dto.getFloorPlans()));
        p.setVirtualTours(joinCsv(dto.getVirtualTours()));
        p.setDocuments(joinCsv(dto.getDocuments()));
    ...
    private List<String> splitCsv(String csv) {
        ...
        for (String part : csv.split(",")) {
            ...
            if (!trimmed.isEmpty()) res.add(trimmed);
        }
        return res;
    }
    ...
    private void autoGeocodeIfMissing(Property p) {
        // already have coords → nothing to do
        if (p.getLat() != null && p.getLng() != null) return;

        // no backend key configured → skip
        if (googleMapsApiKey == null || googleMapsApiKey.isBlank()) {
            System.out.println("⚠️ No google.maps.api.key configured; skipping geocode.");
            return;
        }
        ...
        try {
            ...
            if (response.statusCode() != 200) {
                System.out.println("❌ Geocode HTTP failed: " + response.statusCode());
                return;
            }
            ...
            if (!"OK".equals(status)) {
                System.out.println("❌ Geocode status not OK: " + status
                        + " | error_message=" + json.path("error_message").asText());
                return;
            }
            ...
            System.out.println("❌ Failed to geocode '" + address + "': " + e.getMessage());
        }
    }
