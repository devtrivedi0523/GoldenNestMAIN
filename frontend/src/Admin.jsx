...
import { getAccessToken } from "./auth"; // ✅ NEW
...
export default function AdminDashboard() {
  ...
  // ✅ NEW: uses /api/properties/dashboard/summary (area filtered for AGENT, global for ADMIN)
  const loadSummary = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/properties/dashboard/summary`, {
        headers: { ...authHeaders },
        credentials: "include",
      });
  ...
  // ✅ NEW: uses /api/properties/dashboard?status=...
  const loadList = async (statusKey) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/properties/dashboard?status=${statusParam}&page=0&size=50`,
        {
          headers: { ...authHeaders },
          credentials: "include",
        }
      ...
      if (!res.ok) {
        ...
        throw new Error(`Failed to load ${statusKey} (${res.status}): ${txt}`);
      }
      const data = await res.json();
      setLists((prev) => ({ ...prev, [statusKey]: data.content || [] }));
    } catch (err) {
      console.error(err);
  ...
  // keep approve/decline endpoints as-is (admin endpoints)
