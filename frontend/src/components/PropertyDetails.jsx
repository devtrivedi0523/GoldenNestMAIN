...
export default function PropertyDetails() {
    ...
    if (error || !property) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
                <h1 className="text-2xl font-semibold">{error || "Property not found"}</h1>
                <button onClick={() => navigate(-1)} className="underline text-gray-700">
                    Back to listings
                </button>
            </div>
        );
    ...
    const virtualTours = Array.isArray(property.virtualTours) ? property.virtualTours : [];
    const documents = Array.isArray(property.documents) ? property.documents : [];

    const addressParts = [property.address1, property.city, property.state, property.zip].filter(Boolean);
    const fullAddress = addressParts.join(", ") || locationLine || property.title || "";
                        ...
                        {locationLine && (
                            <p className="text-gray-700 mt-1 font-medium">
                                {formattedPrice && (
                                    <>
                                        {formattedPrice}
                                        <span className="font-normal text-gray-600"> Offers in the Region of</span>
                                        <br />
                                    </>
                                )}
                                {property.bedrooms != null && (
                        ...
                        <div className="border border-gray-200 rounded-lg bg-white">
                            ...
                            <div className="border-b border-gray-200 flex flex-wrap">
                                {[
                                    ...
                                    { id: "floor", label: `Floor Plans (${floorPlans.length})` },
                                    { id: "virtual", label: `Virtual Tours (${virtualTours.length})` },
                            ...
                            <div className="p-5 md:p-6 text-sm text-gray-800">
                                {activeTab === "details" && (
                                    <>
                                        ...
                                        <h3 className="text-lg font-semibold mb-2">Leasehold Information</h3>
                                        <hr className="border-gray-200 mb-3" />
                                        <div className="space-y-1 mb-8 text-sm">
                                            <p><span className="font-semibold">Tenure:</span> {property.tenure || "Not specified"}</p>
                                            <p><span className="font-semibold">Lease Start Date:</span> {property.leaseStartDate || "Not specified"}</p>
                                            <p><span className="font-semibold">Original Lease Term:</span> {property.leaseTermYears != null ? `${property.leaseTermYears} years` : "Not specified"}</p>
                                            <p><span className="font-semibold">Lease Expiry Date:</span> {property.leaseExpiryDate || "Not specified"}</p>
                                        </div>
                                        ...
                                        <p className="text-sm text-gray-700 leading-relaxed">
                                            ...
                                            guidance of intending purchasers and do not constitute an offer or part of a contract. Any services,
                                            heating systems or appliances have not been tested and no warranty can be given or implied as to their
                                            working order. Prospective purchasers should rely on their own inspections and seek appropriate
                                            professional advice.
                                        </p>
                                    </>
                                )}
                                ...
                                {activeTab === "virtual" && (
                                    <div className="text-sm text-gray-600">
                                        ...
                                        {virtualTours.length === 0 ? (
                                            <p>No video or virtual tours are currently available for this property.</p>
                                        ) : (
                                            <ul className="list-disc pl-5 space-y-1">
                                ...
                                {activeTab === "docs" && (
                                    <div className="text-sm text-gray-600">
                                        ...
                                        {documents.length === 0 ? (
                                            <p>Supporting documents (brochures, PDFs, etc.) will appear here once they are added.</p>
                                        ) : (
                                            <ul className="list-disc pl-5 space-y-1">
                                                {documents.map((url, i) => (
                                                    <li key={url + i}>
                                    ...
                                    <div className="text-sm text-gray-600 space-y-4">
                                        ...
                                        {!MAPS_KEY ? (
                                            <p className="text-red-600">
                                                Google Maps key is not configured. Add <code>VITE_GOOGLE_MAPS_KEY</code> to your environment.
                                            </p>
                                        ) : (
                                            <>
                        ...
                        <div className="sticky top-6">
                            <div className="border border-gray-200 rounded-lg bg-white overflow-hidden shadow-sm">
                                ...
                                <div className="px-4 py-4">
                                    ...
                                    {formattedPrice && (
                                        <div className="mb-4">
                                            <p className="text-xs text-gray-500 uppercase tracking-wide font-medium">Guide Price</p>
                                            <p className="text-xl font-bold text-gray-900 mt-0.5">{formattedPrice}</p>
                                        </div>
                                    )}
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => navigate(`/buy/properties/${id}/enquire`)}
                                            className="flex items-center justify-center gap-2 w-full bg-white border border-[#F3B03E] hover:bg-[#F3B03E]/5 text-[#F3B03E] font-semibold text-sm py-2.5 px-4 rounded-lg transition-colors"
                                        >
