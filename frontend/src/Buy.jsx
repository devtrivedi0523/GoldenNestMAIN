...
import {
  FiMapPin, FiHome, FiBox, FiCalendar, FiSearch,
} from "react-icons/fi";
  ...
  return (
    <div className="w-full px-1">
      ...
      <div className="relative h-2">
        ...
        <div ref={rangeRef} className="absolute top-0 h-2 bg-[#F3B03E] rounded-full" />
        ...
        <input
          ...
          className="absolute w-full h-2 appearance-none bg-transparent pointer-events-none [&::-webkit-slider-thumb]:pointer-events-auto [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-[#F3B03E] [&::-webkit-slider-thumb]:shadow-md"
...
const Buy = () => {
  ...
  useEffect(() => {
    ...
    if (minSize || maxSize) {
      if (+minSize >= 3000)      setSize("3000+");
      else if (+minSize >= 2000) setSize("2000-3000");
      else if (+minSize >= 1000) setSize("1000-2000");
      else if (minSize !== "")   setSize("under-1000");
      else setSize("");
    } else setSize("");

    setAppliedFilters({ city, q, type, yearBuilt, minSize, maxSize, minPrice: minP, maxPrice: maxP });
  ...
  const handleSubmit = (e) => {
    ...
    // Pass values directly — don't wait for URL round-trip
    setAppliedFilters({
      city:      params.city      || "",
      q:         params.q         || "",
      type:      params.type      || "",
      ...
      minPrice:  minPrice > MIN_PRICE ? String(minPrice) : "",
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-5xl">
          ...
          <div className="bg-[#F3B03E] rounded-b-lg px-4 py-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            ...
            <div className="flex items-center bg-white rounded-md px-3 py-2">
              <FiMapPin className="mr-2 font-bold shrink-0" />
              <select
